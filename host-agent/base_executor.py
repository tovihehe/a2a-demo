...
class ADKAgentExecutor(AgentExecutor):
    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        # Context: additional info about the state
        # EventQueue: add events to the queue to manage the workflow
...
#### CREATE THE BASE STRUCTURE OF THE SERVERS
def create_agent_a2a_server(
    ...
    # Agent card (metadata) - business card that tells us what the agent can do
    agent_card = AgentCard(
        name=name,
        description=description,
        url=resolved_url, # url of the agent server
        version="1.0.0",
        defaultInputModes=["text", "text/plain"],
        defaultOutputModes=["text", "text/plain"],
        capabilities=capabilities, # agent can do (streaming, pushNotifications)
        skills=skills,
    )
    ...
    # default request handler that wraps the executor (agent)
    request_handler = DefaultRequestHandler(
        agent_executor=executor,
        task_store=InMemoryTaskStore(),
    )

    # Create A2A application instance
